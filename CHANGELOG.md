...
- 发布改为在 GitHub 网页 Actions 中一键执行，不再要求开发电脑能够运行 Git 命令
- 云端自动创建正确版本标签、Release、签名安装包、签名文件和更新清单
...
- Windows 桌面版支持应用内自动更新：发现新版后可直接下载、校验、安装并自动重启
- 设置页新增完整的检查更新、更新内容、下载进度和失败原因提示

## 🔧 修复

- 统一应用显示版本、安装包版本和更新器版本来源，避免更新后版本号不一致
- GitHub Release 自动发布签名安装包、签名文件和更新清单，避免“检测到更新但无法安装”
...
- 修复 3.24.2 构建失败的问题，恢复完整多语言翻译文件
- 保留“查看最新版”下载入口，避免应用内检测失败提示
...
- 将桌面端“检查更新”改为直接打开最新版下载页，避免无签名更新模式下的网络检测失败提示
...
- 修复 Windows Release 的 Tauri CLI 调用，使 GitHub Actions 能正常构建并上传安装包
- 统一桌面端、前端与 Rust 的版本号为 3.24.1
...
本版本对桌面端视觉语言进行打磨，优化了各区域的颜色、边框与层级表现，使界面更沉静的桌面美学。

---

## 🔧 变更

- 为各主要区域（日历、规划、设置、统计）添加语义化 CSS 类名，使样式更可精确控制
- 日历时间网格、事件块新增数据属性，便于主题扩展
- 规划页容器、任务行、列表头、完成区优化样式层级
- 设置页与统计页增加上下文 class，提升视觉一致性
- 优化桌面端的整体色彩层次与边框质感

## [v3.24.0]: https://github.com/vanemacus486-bit/CaILens/releases/tag/v3.24.0
