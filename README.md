...
> **状态：** v3.17.0 内联展开编辑、描述优先写作流、Serif 阅读日视图、Markdown 渲染、搜索片段命中、悬浮预览、暗色对比度审计、会话状态恢复。955 个测试。
...
- **合并组系统**——可自定义任意分类合并为合计线（如"投入合计" = 主要矛盾 + 次要矛盾），堆叠面积 + 粗实线总计，独立开关；右键 chip 弹出配置面板（命名、选分类），配置持久化到 localStorage。
