...
> **Android APK 尚未通过 Google Studio 重新打包。** 下载的 APK 仍然是旧版本，不包含本次移动端响应式改动。桌面端 Web 版已适配响应式布局（侧边栏汉堡菜单、弹性间距、触控支持），可在手机浏览器中直接使用。如需完整体验，请使用 Windows 桌面版。
...
- **跨天事件**——睡眠等跨越午夜的记录自动跨天显示，带箭头指示器和连续圆角。（⚠️ 实验性功能，存在已知 bug。跨天事件的拖拽和跨天 resize 行为可能不符合预期。）
...
如果强制浏览器开启深色模式，那么画面可能会呈现橙色，后续会添加深色模式

跨天事件目前还不能很好支持，可能会出现时间块消失的问题
