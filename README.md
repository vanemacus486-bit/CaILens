...
- **标题优先输入**——打开即 autofocus 标题框。Enter 保存并自动将焦点移至 description 框——写描述成为自然的下一步。再按 Enter 保存所有内容并关闭，按 Esc 随时退出（事件已保存）。
- **可折叠详情**——description + location，默认折叠，Enter 保存标题后自动展开，200ms `max-height` 动画。
