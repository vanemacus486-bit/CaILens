...
> **状态：** v3.8 统计页面重设计——洞察优先的"赤字仪表盘"（Deficit Dashboard）+ 小多图矩阵（Small Multiples）。378 个测试。
...
- **代码质量优先于功能数量。** TypeScript 严格模式，378 个测试，单向依赖分层。代码库应该经得起时间考验。
...
点击侧边栏图表图标进入。**顶部视图切换器**选择视图，一次只显示一个。**周期选择**（周 / 月 / 季 / 年）切换粒度：

**诊断视图**（默认）
- 进入界面第一眼不是数据，而是诊断——算法先告诉你时间花得对不对。
- **本周诊断卡片**——三条洞察：最大问题（如"睡眠欠债 44h"）、做得最好的方面、值得思考的问题。
- **预算对账图**——对称条形图，目标线为原点。欠债向左、超额向右，颜色代表状态（红=严重欠债、橙=不足、绿=达成、焦橙=超额），一眼判断每个分类的位置。
- **趋势火花线**——上期 vs 本期总量变化，手绘 SVG 小图。
- **节奏匹配卡**——7×24 小时分布热力小图 + 自动计算"今天最像周几"。

**分项对比视图**
- 3×2 栅格，每个分类一个独立迷你折线/柱状图（手绘 SVG，无第三方库）。
- 虚线标记预算目标线。折线形状直接揭示模式——上升、平台、骤降。
- 支持 2/3/6 期对比，折线/柱状切换。每图底部状态评语 + 变化量。
- 可以并行扫视——1 秒内得到 6 个分类的判断。
...
| 测试 | Vitest 4 + React Testing Library + fake-indexeddb | 378 个测试，22 个测试文件 |
