...
**全局快捷键：** `1`→日历, `2`→规划, `3`→复盘, `Esc`→回周视图, `N`→快速记录, `Ctrl+K`→搜索, `Ctrl+C/V`→复制/粘贴事件, `←/→`→上下文导航（周视图=周, 复盘=时段, 规划=筛选循环）, `Ctrl+Shift+←/→`→上/下天, `M`→切换周/月视图
...
**5 视觉风格** (graphite/nocturne/carbon/tide/indigo)，通过 `<html data-style="...">` 切换（`settingsStore.ts` 写入，`index.html` 启动还原）。定义在 `tokens.css` 末尾的 `[data-style]` 覆盖块，同时改写两套 token 体系（index.css 的 `--surface-*`/`--accent*` 与 tokens.css 的 `--paper`/`--ink*`/`--line*`），不影响事件分类色。`graphite` = 项目默认暖色；`tide`/`indigo` = Timepage 式全画布深底沉浸风格（浅色模式也是深底，需覆盖 `--text-*`）。
